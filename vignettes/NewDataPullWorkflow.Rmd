...
output: rmarkdown::html_vignette:
        toc: true
vignette: >
  %\VignetteIndexEntry{NewDataPullWorkflow}
...
View(locationeffort)
View(locationeffort[[7]][[1]]) #View nested effort data for camera
```
...
locationeffort2 <- rm_bad_locations(locationeffort = locationeffort, coordinate_precision = coord_prec)
```
...
```{r}
detections <- query_detections(conn = conn, species = "Bear", grid = "SSWI", daterange = daterange2, prec = 0)
```
...
Q4 <-create_sampling_occasions(daterange = daterange2, locationeffort = Q3, num_occasions = 4, class_threshold =  0.95)
...
day_occasion_df_table <- daterange2 %>% dplyr::group_by(year) %>% 
...
Q5 <- summarize_detections(detections = detections, locationeffort = Q4, summary_value = "max count")
...
Q5.ct <- summarize_detections(detections = detections, locationeffort = Q4, summary_value = "count triggers")
...
effort_by_occ_df_maxcount_sf = Q5 %>%
...
effort_by_occ_df_counttriggers_sf = Q5 %>%
  sf::st_as_sf(coords=c("lon", "lat"), crs=4326)%>%
  sf::st_join(
    .,
    sf::st_transform(get_spatial_data("bear_zones"), 4326),
