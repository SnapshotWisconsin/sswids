...
list_nolocs <- rm_noloc_data(locs_df = locs_df_raw, effort_df = effort_df_raw, detections_df = detections_df_raw, "ruffed_grouse_priority_areas", rugr_pr)
```
...
list_removelowpreclocs <- remove_low_precision_lat_long(list_outsideactivedates, coordinate_precision=coordinate_precision)
```

## Spatially subset camera locations (optional)

There may be times where it doesn't make sense to include data from the entire Snapshot Wisconsin camera network, and we should spatially subset camera locations. For example, we may want to only consider the Northern Forest Zone instead of the entire state of Wisconsin. We can filter locations, detections, and effort based on a spatial layer, like deer management unit. We provide example code in the 'Additional resources' section of this document. 
...
list_camsiteid <- detect_remove_overlap(list_camsiteid)
```
...
detections_df_precision = detections_df_removeoverlap %>%
  group_by(trigger_id) %>%
...
effort_by_day_df <-
  sswids::effort_by_day(effort = effort_df_removeoverlap)
```
...
mean_locs_df <-
  sswids::average_camera_coordinates(
    locations = locs_df_removeoverlap,
...
# use all data by setting proportion classified to 0
 ppn_class_threshold = 0
effort_by_occ_df <-
    calculate_prop_classified_updated(
      seasons = seasons_df,
...
There are various ways we can do this. The default is using the max count of every `CLASS_KEY` per `cam_site_id-year-occasion`. For example, for `DEER_ADULT_ANTLERLESS_AMT`, the max count for cam_0001 in 2019 occasion 1 is 1. The max count is 0 if there are any days of effort but 0 detections. The max count is NA if there are 0 days active. 

A different function will calculate the total number of triggers in each `CLASS_KEY` by `cam_site_id-year-occasion` combo. For example, for `DEER_ADULT_ANTLERLESS_AMT`, the total triggers for cam_0001 in 2019 occasion 1 is 7. In this row of data there were 7 triggers that were classified as `DEER_ADULT_ANTLERLESS_AMT`. This structuring of the data makes the total counts of `CLASS_KEYS` more comparable to raw data.

**TO DO:**
*Make a function in `sswids` to calculate total number of triggers of `CLASS_KEY` by occasion.*
...
summarize_detections_counttriggers <- function(detections, seasons) {
  ...
  detections <-
    ...
    # COUNT instead of MAX, changed all counts >0 to 1, so sum should work here. 
    dplyr::summarise(dplyr::across(matches("[A-Z]", ignore.case = FALSE), sum, na.rm = TRUE)) %>%
...
detections_wide_df_counttriggers <-
  summarize_detections_counttriggers(
    detections = detections_df_precision,
    seasons = seasons_df
  )
...
dets_orig = detections_df_raw %>%
  ...
  count() %>%
  rename("total" = "n")
dets_noloc = detections_df_nolocs %>%
  group_by(class_key) %>%
  count() %>%
  rename("n-no_loc" = "n")
dets_baddate = detections_df_outsideactivedates %>%
  group_by(class_key) %>%
  count() %>%
  rename("n-bad_date" = "n")
dets_lowlocprec = detections_df_removelowpreclocs %>%
  group_by(class_key) %>%
  count() %>%
  rename("n-low_loc_prec" = "n")
dets_effortoverlap = detections_df_removeoverlap %>%
  group_by(class_key) %>%
  count() %>%
  rename("n-effort_overlap" = "n")
dets_precision = detections_df_precision %>%
  group_by(class_key) %>%
  count() %>%
...
layer_map <- 
  sswids::get_spatial_data(layer_name = 'dmus') %>%
  mapview(zcol = 'deer_management_zone', layer.name = 'Deer Management Zones')
